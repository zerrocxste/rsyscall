...
long remote_mmap(int pid, void *address, size_t length, int prot, int flags, int fd, off_t offset)
{
    ...
    auto proc_syscall = parse_procfs_syscall(fd_syscall);
    ...
    read_memory(fd_mem, proc_syscall.rip, backup, sizeof(backup));
    write_memory(fd_mem, proc_syscall.rip, code, sizeof(code));
    ...
    while (try_count < MAX_TRY_COUNT)
    {
        ...
        read_memory(fd_mem, proc_syscall.rip, patch, 2);
    ...
    while (true)
    {
        ...
        if (fd_syscall <= 0)
            return -ESRCH;
        kill(pid, SIGSTOP);
        proc_syscall = parse_procfs_syscall(fd_syscall);
